...
# Training data
# dataset = np.loadtxt('datasets/gendata_2mut_train_pred_aucune_rep=500.txt', delimiter=" ")
dataset = np.loadtxt('datasets/gendata_2mut_train_all_rep=500.txt', delimiter=" ")

X = dataset[:, 4:29]        # take 25 stats (the last one, ie. the difficulty, is discarded)
Y = dataset[:, 0:4]         # 4 params for this model
# Normalization by column: slightly worse than no scaling at all;
# it achieves map error above 9% % (vs 8-9% without any scaling).
...
# dataset_test = np.loadtxt('datasets/gendata_2mut_test_pred_aucune_rep=500.txt', delimiter=" ")
dataset_test = np.loadtxt('datasets/gendata_2mut_test_all_rep=500.txt', delimiter=" ")

X_test = dataset_test[:, 4:29]
y_test = dataset_test[:, 0:4]
...
for i in range(n_models):
    # Reproducibility
    # np.random.seed(5705*i)
    # rn.seed(5705*i)
    # tf.set_random_seed(574137*i) 
    ...
    if score[1] < best_score:
        ...
        best_score = score[1]
        model.save('model2_est_param.h5')
